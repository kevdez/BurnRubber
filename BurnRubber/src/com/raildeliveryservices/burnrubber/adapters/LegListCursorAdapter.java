...
public class LegListCursorAdapter extends CursorTreeAdapter  {
	...
	protected void bindChildView(View view, Context context, Cursor cursor, boolean isLastChild) {
		...
		final boolean countFlag = cursor.getInt(cursor.getColumnIndex(Leg.Columns.COUNT_FLAG)) == 1 ? true : false;
		final boolean weightFlag = cursor.getInt(cursor.getColumnIndex(Leg.Columns.WEIGHT_FLAG)) == 1 ? true : false;
		final boolean outboundFlag = cursor.getInt(cursor.getColumnIndex(Leg.Columns.OUTBOUND_FLAG)) == 1 ? true : false;
